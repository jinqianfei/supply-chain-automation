...
# 使用方法:
#   1. 首次使用：填写下方配置，然后执行:
#      bash scripts/sync_to_aliyun.sh --setup
#      （会设置 SSH 免密登录，之后不再需要密码）
#
#   2. 日常同步:
#      bash scripts/sync_to_aliyun.sh
#
...
# 本地工作区路径
LOCAL_WORKSPACE="/Users/jinqianfei/openclaw-workspaces/ai-order"
...
do_sync() {
    ...
    if [ -n "$PENDING_SQL" ]; then
        ...
        remote_exec "cd $ALIYUN_WORKSPACE && bash scripts/run_migrations.sh" 2>&1
        ok "数据库迁移完成"
    else
        ok "无数据库迁移"
    fi
...
case "${1:-sync}" in
    ...
    --help|help|-h)
        echo "用法:"
        echo "  bash scripts/sync_to_aliyun.sh          # 日常同步"
        echo "  bash scripts/sync_to_aliyun.sh --setup   # 首次: 设置 SSH 免密"
        echo "  bash scripts/sync_to_aliyun.sh --help    # 帮助"
        ;;
    *)
        do_sync
        ;;
