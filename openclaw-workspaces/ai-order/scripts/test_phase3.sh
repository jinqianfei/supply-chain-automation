...
#
# 覆盖：
# 1. reindex_memory.py（build / check / query）
# 2. extract_memory.py（生成摘要 / --apply 替换）
# 3. check_memory_quality.py（4W 检查 / 质量分）
...
# ────────────────────────────────────────
sep "Test 1: reindex_memory.py — 重建索引"
OUT=$(python3 "$SCRIPTS/reindex_memory.py" 2>&1)
EC=$?
...
# ────────────────────────────────────────
sep "Test 2: reindex_memory.py — 健康检查"
OUT=$(python3 "$SCRIPTS/reindex_memory.py" --check 2>&1)
EC=$?
...
# ────────────────────────────────────────
sep "Test 3: reindex_memory.py — 搜索 version_check"
OUT=$(python3 "$SCRIPTS/reindex_memory.py" --query "version_check" 2>&1)
EC=$?
...
# ────────────────────────────────────────
sep "Test 4: reindex_memory.py — 搜索中文"
OUT=$(python3 "$SCRIPTS/reindex_memory.py" --query "断档" 2>&1)
EC=$?
...
# ────────────────────────────────────────
sep "Test 7: check_memory_quality.py — 全量检查"
OUT=$(python3 "$SCRIPTS/check_memory_quality.py" 2>&1)
EC=$?
...
# ────────────────────────────────────────
sep "Test 8: check_memory_quality.py --file 单个文件"
OUT=$(python3 "$SCRIPTS/check_memory_quality.py" --file memory/2026-06-08.md 2>&1)
EC=$?
...
# ────────────────────────────────────────
sep "Test 9: check_memory_quality.py --strict"
OUT=$(python3 "$SCRIPTS/check_memory_quality.py" --strict 2>&1)
EC=$?
...
OUT=$(bash "$SCRIPTS/phase3_maintenance.sh" --dry-run 2>&1)
EC=$?
