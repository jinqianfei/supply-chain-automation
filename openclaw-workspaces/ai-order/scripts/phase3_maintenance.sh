...
#
# 执行内容：
#   1. memory_search 索引重建（reindex_memory.py）
#   2. MEMORY.md 自动提取（extract_memory.py --apply）
...
#   bash scripts/phase3_maintenance.sh --skip-reindex  # 跳过 reindex
#   bash scripts/phase3_maintenance.sh --dry-run      # 不写任何文件
...
if [ "$SKIP_REINDEX" = false ]; then
  ...
  if [ "$DRY_RUN" = true ]; then
    ...
    python3 "$SCRIPTS/reindex_memory.py" 2>&1 | tail -5
  fi
  echo ""
fi
...
echo "▶ Step 3: 4W 质量检查"
python3 "$SCRIPTS/check_memory_quality.py" --strict 2>&1 | tail -10
QUALITY_EXIT=$?
echo ""
