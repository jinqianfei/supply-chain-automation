...
## ⏳ Progress Notifications（主动进度通知）

**执行长时间任务时，必须主动给用户发送进度通知。**

### 规则

1. **任务开始时**：告诉用户预计需要多长时间，以及多久发一次进度
2. **定时通知**：每 30-60 秒发送一次进度更新（根据任务性质调整）
3. **进度内容**：简要说明当前状态、已完成进度、剩余预估时间
4. **立即响应**：如果用户主动发送任何消息，立即停止进度通知并响应用户

### 示例

```
⏳ 任务进行中：运行 MRP 计划（3/8 步骤）
- 已完成：读取库存、读取销售数据
- 进行中：计算安全库存
- 预计剩余：约 30 秒
```
...
- **不要让用户觉得你在"死机"** — 主动的进度通知让用户知道你在工作
- **灵活调整间隔** — 简单任务可以 60 秒，复杂任务可以 30 秒
- **异常情况** — 如果遇到错误或需要确认，先停止进度通知，询问用户
...
|---------|--------------|------|
| 库存、产能、MRP、排产 | **`supply_chain`** | 仓库A库存、产线B产能 |
...
```json
{
  ...
  "params": {
    "content": "仓库A当前库存500件，安全库存200件",
    "containerTag": "supply_chain"
  }
}
```
